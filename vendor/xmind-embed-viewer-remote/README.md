...
本目录保存 `https://www.xmind.app/embed-viewer` 在 2026-06-10 17:14:30 CST 抓取的调试快照，用于排查 Obsidian 插件通过 XMind embed viewer 渲染本地 `.xmind` 文件时的问题。正式插件构建会从本目录读取资源并内联进 `main.js`，运行时不依赖安装目录中存在本目录。

## 目录说明

- `local/embed-viewer.html`：调试用本地 iframe 入口，使用相对路径加载本目录镜像资源。
...
- 内联动态资源路径：主 bundle 读取 `window.__XMIND_ASSET_MAP__`，由 `src/core/xmind-viewer-assets.ts` 指向运行时 Blob URL。
- 中心主题兼容修复：主 bundle 中的 `xmindNormalizeLocalOpenFile` 会在本地 `open-file` 载入前修正新版 XMind 文件里不可见的中心主题继承色。

## 注意事项

这是一份从远程 iframe 抓取并本地化的资源快照。插件构建会把必要资源内联进 `dist/main.js`，运行时通过本地 Blob iframe 加载，不再创建 `https://www.xmind.app/embed-viewer` 远程 iframe。
