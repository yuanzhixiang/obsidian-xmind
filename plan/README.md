...
`README.md` 是仓库默认展示的英文用户说明书，面向 Obsidian 用户介绍 `XMind Maps` 的安装、打开文件、Markdown embed、查看控件、搜索、大纲、分支展开/收起、XMind 内容展示、面板菜单和只读限制。
...
- 查看器控件章节必须覆盖缩放、适配、刷新、搜索、大纲、sheet 切换、滚轮或触摸板移动、触摸板缩放、`Command + wheel` 缩放和右键拖拽平移。
- 分支章节必须说明按 XMind 文件保存的折叠状态初始化、点击数字圆圈展开、hover 已展开主题或连接线显示收起控件、`> 999` 显示 `...`。
- 搜索章节必须说明搜索当前 sheet 的主题标题、标签、备注和链接，支持 `Command+F` / `Ctrl+F` 唤醒、下一个/上一个跳转，并会临时展开折叠祖先让结果可见。
