...
该模块把 `vendor/xmind-embed-viewer-remote` 中运行本地 XMind viewer 必需的 CSS、JS、动态 chunk、图片和动画资源打包进插件 `main.js`，运行时生成 Blob/Data URL，并返回 iframe 可加载的本地 HTML Blob URL。
...
- JS：jQuery、polyfill、js-cookie、Popper、Bootstrap、Vue、修复后的 `share-embed`、Snowbrush。
...
- 新增 XMind chunk 或资源时，必须同步更新本模块和 `scripts/check-xmind-local-viewer-fix.mjs`。
